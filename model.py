...
for col in bool_cols:
    if col in data.columns:
        data[col] = (
            data[col].astype(str).str.lower().map({"yes": 1, "no": 0})
        )
...
axes[0].axvline(data['score'].mean(), color='red', linestyle='--')
axes[0].axvline(data['score'].median(), color='green', linestyle='--')
...
if len(cat_features) == 0:
    cat_features = [c for c in numeric_cols if data[c].nunique() < 10 and c != 'score']
...
train_sizes, train_scores, test_scores = learning_curve(best_pipeline, X_train, y_train, cv=5,
...
if 'Random Forest' in trained_models:
    ...
    if len(cat_features) > 0:
        ...
        feature_names.extend(encoder.get_feature_names_out(cat_features))
...
for lin_name in ['Linear Regression', 'Ridge Regression']:
    if lin_name in trained_models:
        ...
        plt.tight_layout()
        plt.savefig(f'EX_coefs_{lin_name.replace(" ","_")}.png', dpi=300, bbox_inches='tight')
        plt.close()
